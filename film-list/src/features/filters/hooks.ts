import { MoviesFilterSchema, type GenreFilter, type GenreName, type MoviesFilter } from '@/entities/movie/model/movie';
import { useMemo } from 'react';
import { useSearchParams } from 'react-router-dom';

function parseGenres(raw: string[]): { include: GenreName[]; exclude: GenreName[] } {
 ...
 const getGenres = (genresObject: GenreFilter) => [...genresObject.include.map(g => `+${g}`), ...genresObject.exclude.map(g => `!${g}`)]
...
export function searchParamsToFilters(
  ...
  const p =
    params instanceof URLSearchParams ? params : new URLSearchParams(params as any);
...
export const useFilters = (): {filters: MoviesFilter, setFilters: (filters: MoviesFilter) => void, clearFilters: () => void} => {
  ...
  const filters: MoviesFilter = useMemo(() => searchParamsToFilters(searchParams), [searchParams])
