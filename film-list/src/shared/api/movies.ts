...
import { MovieCardSchema, MovieDetailsSchema, type Movie, type MovieDetails, type MoviesFilter } from "@/entities/movie/model/movie";
import { filtersToSearchParams } from "@/features/filters";


const FIELDS_CARD: string[] = ['id', 'name', 'year', 'poster', 'rating'];
const NOT_NULL_FILEDS: string[] = ['id', 'name', 'year', 'poster.url', 'rating.kp', 'genres.name'] //на всякий случай добавил жанры, описание не стал добавлять хотя тоже можно
const FIELDS_DETAIL: string[] = ['id', 'name', 'year', 'poster.url', 'rating.kp', 'description', 'genres.name'];


export type InfiniteMovies = { items: Movie[], nextPage: number | null, prevPage: number | null }
...
export async function fetchMovie(id: string, signal?: AbortSignal): Promise<MovieDetails> {
