...
export class MoviesData<T extends InfiniteMovies> {
  ...
  getMovie = (index: number): Movie | null => {
    ...
    const moviePage =
      this.data.pages[this.data.pageParams.indexOf(moviePageIndex)];
  ...
  getRealItemsCount = (): number => {
    if (!this.data) throw new Error("No data");
    return (
      this.data.pageParams.length -
      1 * MOVIES_PAGE_LIMIT +
      this.data.pages.at(-1)!.items.length
    ); //одна страница точно есть число элементов на последней может быть меньше 50
  };

  getPotentialItemsCount = (): number => {
    if (!this.data) throw new Error("No data");
    return (
      (this.getLastPage() + this.preloadPagesCount - 1) * MOVIES_PAGE_LIMIT +
      this.data.pages.at(-1)!.items.length
    ); //одна страница точно есть число элементов на последней может быть меньше 50
  };
